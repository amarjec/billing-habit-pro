...
const ManageProducts = () => {
    ...
    // ✅ NEW: Track if we have completed at least one fetch to prevent empty state flash
    ...
    const fetchProducts = async (currentPage, search, catId, subCatId, reset = false) => {
        ...
        try {
            ...
            setLoading(false);
            // ✅ Mark that we have attempted a fetch, so "No Products" can now safely show if needed
    ...
    return (
        <div className="pb-24 bg-gray-50 min-h-screen">
            ...
            <div className="px-4 py-4 space-y-3">
                {/* ✅ LOGIC FIX: Only show Empty state if we have fetched at least once AND list is empty */}
                {products.length === 0 && !loading && !initialLoading && hasFetchedOnce ? (
                    ...
                    <>
                        ...
                        {/* ✅ LOGIC FIX: Show Skeleton if loading OR if we haven't fetched the first batch yet */}
                        {(loading || initialLoading || !hasFetchedOnce) && (
                            <ProductSkeleton />
                        )}
